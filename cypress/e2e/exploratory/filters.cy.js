...
beforeEach(() => {
  ...
  describe('Filters Orders overview', () => {
    ...
    it('Account', () => {
      ...
      // Ellipsis should be visible and clickable
      cy.get('[data-column="account"] [data-icon="ellipsis"]').should('be.visible').click()
      
      // Account searchable dropdown should be visible
      ...
      // Click dropdown button should see all the dropdown values
      cy.get('[data-column="account"] [data-satis-dropdown-target="toggleButton"]').click()
      
      // Validate elements of filter card
      ...
      // Comment SH: Please include a test for auto select and partial search.
    ...
    it('TYPE Filter', () => {
      ...
      // Type searchable dropdown should be visible
      ...
      // Comment SH: Maybe we should create a loop that checks the first 25 rows
    ...
    it.only('STATE Filter', () => {
      ...
      // Comment SH: I see that the delay 500 is still too vast, maybe we should update it?
      cy.wait(500)
      cy.contains('[title*="index.orders"]', 'Orders').click()
